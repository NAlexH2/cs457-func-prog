...
Note: the following definition is not graded, but you should try implementing
...
If you look up the definition of `guard`, you will probably see a constraint
called `Alternative` that we haven't seen before. We will talk about it later in
this file! For now, you can just treat it as a black box that only continues if
...
input. If there are *no* characters in the input, then it returns a successful
parse of a unit value and the remaining string (still nil). Otherwise, if there
are any characters at all, this parser fails.
...
Using `get` we can write a composite parser that returns a pair of the first two
`Char` values from the front of the input string:
...
More generally, we can write a *parser combinator* that takes two parsers and
returns a new parser that uses first one and then the other and returns the pair
of resulting values...
...
Suppose we want to parse *two* characters, where the first should be a sign and
the second a digit?

We've already defined single character parsers that should help. We just need to
put them together.
...
And, `Control.Applicative` gives us even more options for constructing parsers.
For example, it also includes a definition of `liftA3`.

> tripleP :: Parser a -> Parser b -> Parser c -> Parser (a,b,c)
> tripleP = liftA3 (,,)


The `*>` and `<*` operators are also defined in `Control.Applicative`. The first
is the `Applicative` analogue of the `(>>)` operator for `Monads`.

< -- sequence actions, discarding the value of the first action
< (*>) :: Applicative f => f a -> f b -> f b

The second is the dual to the first---it keeps the first result but discards the
second.
...
The `Parser` type is also a `Monad`. Just like the state monad and list, we can
make `Parser` an instance of the `Monad` type class. See if you can figure out
...
However, to parse more interesting things, we need to add some kind of recursion
to our combinators. For example, it's all very well to parse individual
characters (as in `char` above), but it would a lot more fun if we could
recognize whole `String`s.
...
The `Applicative` operators give us sequential composition of parsers (i.e. run
one parser then another). But what about parallel composition (i.e. run both
parsers on the same input)?
...
In the definition of `chooseFirstP`, note how we duplicate the input string `s`
and give the same string to both parsers. This code naturally implements
backtracking. If the first parser fails, we go back to the state of the input
where it started and then continue with the second parser.

Example: We can use the above combinator to build a parser that returns either
...
`p`) and returns a *list* of `a` values. That is, it keeps grabbing `a` values
as long as it can and returns them in a list of type `[a]`.
...
We can use choice and failure together to make the `Parser` type an instance of
the `Alternative` type class from
...
"choice" operator that combines two computations. The `Alternative` type class
laws require the choice operator to be associative but it need not be
commutative (and it isn't here).

The `empty` computation should be an identity for the choice operator. In other
...
The `Alternative` type class automatically gives definitions for functions
`many` and `some`, defined in terms of `(<|>)`.

The `many` operation corresponds to running the applicative computation zero or
...
For parsing, the `many` combinator returns a single, maximal sequence produced
by iterating the given parser, zero or more times
...
starting with a multiplication expressions (`mulE`). Multiplication expressions
must then start with a basic factors: either natural numbers or arbitrary
...
Instead, we want to parse the input as starting with a multiplication expression
followed by any number of addition operators and multiplication expressions. We
can temporarily store the operators and expressions in a list of pairs. Then,
we'll `foldl` over this list, using each operator to combine the current result
...
Also, it is very easy to spot and bottle the chaining computation pattern: the
only differences are the *base* parser (`mulE1` vs `factorE1`) and the binary
operation (`addOp` vs `mulOp`). We simply make those parameters to our
...
This concludes our exploration of applicative parsing, but what we've covered is
really just the tip of an iceberg. Though parsing is a very old problem, studied
since the dawn of computing, algebraic structures in Haskell bring a fresh
perspective that has now been transferred from Haskell to many other languages.
